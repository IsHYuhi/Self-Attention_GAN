...
def train_model(G, D, dataloader, num_epochs, save_model_name='model'):
    ...
    for epoch in range(num_epochs+1):
        ...
        for images in tqdm(dataloader):
            ...
            #hinge loss
            #if d_out_real >= 1, d_loss_real would be 0 by ReLU
            d_loss_real = torch.nn.ReLU()(1.0 - d_out_real).mean()
            #if d_fout_fake <= -1, d_loss_fake would be 0 by ReLU
...
def main():
    ...
    num_epochs = 100
    G_update, D_update = train_model(G, D, dataloader=train_dataloader, num_epochs=num_epochs, save_model_name='SAGAN')
